...
public class UserController {
	...
	public R downUser(@RequestBody Tyyh tyyh){
		...
		return new R<>(userService.dowmUser(tyyh));
	}
	...
	public R updateUser(@Valid @RequestBody UserDTO userDto) {
		if(CommonConstants.ADMIN_FLAG.equals(userDto.getUserId())&& !SecurityUtils.getUser().isAdmin()){
			return R.builder()
					.code(CommonConstants.FAIL)
					.msg("您没有权限对该用户进行任何操作！").build();
		}
	 ...
	 */
	@Inner
	@GetMapping("/pki/info/{idCard}")
	public R pkiUser(@PathVariable String idCard) {
		SysUser user = userService.getOne(Wrappers.<SysUser>query()
				.lambda().eq(SysUser::getIdcard, idCard));
		if (user == null) {
			return new R<>(Boolean.FALSE, String.format("用户身份证不存在 %s", idCard));
		}
		return new R<>(userService.findUserInfo(user));
	}
