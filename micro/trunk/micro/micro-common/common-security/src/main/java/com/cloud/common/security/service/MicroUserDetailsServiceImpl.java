...
import com.cloud.admin.api.feign.RemoteWeChatService;
...
public class MicroUserDetailsServiceImpl implements MicroUserDetailsService {
    ...
    private final RemoteWeChatService remoteWeChatService;
    ...
    @Override
    public SysUser loadWeChatUserByACode(String acode) throws UsernameNotFoundException {
        R<SysUser> sysUserR = remoteWeChatService.getUser(acode, SecurityConstants.FROM_IN);
        if (sysUserR.getCode() == 0) {
            return sysUserR.getData();
        } else {
            throw new UsernameNotFoundException(sysUserR.getMsg());
        }
    ...
    private UserDetails getUserDetails(R<UserInfo> result) {
        ...
        return new MicroUser(user.getUserId(), user.getUsername(), user, info.getSysDept(), info.getSysRole(), SecurityConstants.BCRYPT + user.getPassword(), enabled,
