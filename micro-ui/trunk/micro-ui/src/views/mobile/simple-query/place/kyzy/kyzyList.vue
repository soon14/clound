                  ...
                  >
                    <van-card v-for="item in tableData" @click="viewPlaceDetail(item)">
                        <div class="cardList" slot="title">
                            <span style="font-weight: bold;" v-text="item.name"></span>
                        </div>
                        <div class="cardList" slot="desc">
                            <span>{{validatenull(getLabelByVal(placeTypeData,item.type))?'暂无数据':getLabelByVal(placeTypeData,item.type)}}|</span>
                            <span>{{validatenull(getLabelByVal(placeStateData,item.enterpriseState))?'暂无数据':getLabelByVal(placeStateData,item.enterpriseState)}}|</span>
                            <span>{{validatenull(getLabelByVal(areaData,item.areaCode))?'暂无数据':getLabelByVal(areaData,item.areaCode)}}</span>
                        </div>
                        ...
                        <div class="cardList" slot="tags">
                           <span><img src="/img/mobile/location.png">
                               {{validatenull(item.address)?'暂无数据':item.address}}</span>
                        </div>
            ...
            viewPlaceDetail(row){
                this.$router.push({
                    path: this.$router.$avueRouter.getPath({
                        src: "/mobile/simpleQuery/kyzyDetail"
                    }),
                    query: {
                        id: row.id
                    }
