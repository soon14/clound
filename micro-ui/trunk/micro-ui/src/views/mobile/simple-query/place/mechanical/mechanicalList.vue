                  ...
                  >
                    <van-card v-for="item in tableData" @click="viewPlaceDetail(item)">
                        <div class="cardList" slot="title">
                            <span style="font-weight: bold;" v-text="item.dwmc"></span>
                        </div>
                        <div class="cardList" slot="desc">
                            <span>{{validatenull(getLabelByVal(placeTypeData,item.dwlbdm))?'暂无数据':getLabelByVal(placeTypeData,item.dwlbdm)}}|</span>
                            <span>{{validatenull(getLabelByVal(placeStateData,item.qyztdm))?'暂无数据':getLabelByVal(placeStateData,item.qyztdm)}}|</span>
                            <span>{{validatenull(getLabelByVal(placeNatureData,item.jjlxdm))?'暂无数据':getLabelByVal(placeNatureData,item.jjlxdm)}}|</span>
                            <span>{{validatenull(getLabelByVal(areaData,item.dwdzssxqdm))?'暂无数据':getLabelByVal(areaData,item.dwdzssxqdm)}}</span>
                        </div>
                        ...
                        <div class="cardList" slot="tags">
                           <span><img src="/img/mobile/location.png">
                               {{validatenull(item.dwdzqhnxxdz)?'暂无数据':item.dwdzqhnxxdz}}</span>
                        </div>
            ...
            viewPlaceDetail(row){
                this.$router.push({
                    path: this.$router.$avueRouter.getPath({
                        src: "/mobile/simpleQuery/mechanicalDetail"
                    }),
                    query: {
                        id: row.zagldwbm
                    }
