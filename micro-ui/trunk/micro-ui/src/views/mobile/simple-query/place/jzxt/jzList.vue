                ...
                >
                    <van-card v-for="item in tableData" @click="viewPlaceDetail(item)">
                        <div class="cardList" slot="title">
                            <span style="font-weight: bold;" v-text="item.qyzcmc"></span>
                        </div>
                        <div class="cardList" slot="desc">
                            <span>{{validatenull(getLabelByVal(placeTypeData,item.gxhylx))?'暂无数据':getLabelByVal(placeTypeData,item.gxhylx)}}|</span>
                            <span>{{validatenull(getLabelByVal(placeStateData,item.jyzt))?'暂无数据':getLabelByVal(placeStateData,item.jyzt)}}|</span>
                            <span>{{validatenull(getLabelByVal(areaData,item.dwszdxzqh))?'暂无数据':getLabelByVal(areaData,item.dwszdxzqh)}}</span>
                        </div>
                        ...
                        <div class="cardList" slot="tags">
                            <span><img src="/img/mobile/location.png">
                                {{validatenull(item.jydz)?'暂无数据':item.jydz}}</span>
                        </div>
            ...
            viewPlaceDetail(row) {
                this.$router.push({
                    path: this.$router.$avueRouter.getPath({
                        src: "/mobile/simpleQuery/jzDetail"
                    }),
                    query: {
                        id: row.systemId
                    }
