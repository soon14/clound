  ...
  export default {
        ...
        try {
          arr.forEach(a => {
            this.tree.children.push({
              ...
              hasChildren:a.hasChildren==="true"?true:false,
    ...
    computed: {
      ...
      iconCss() {
          let that = this;
          return function () {
             return that.tree.hasChildren?that.tree.open? "ico_open":"ico_close": "ico_docu";
        }
      },
      load() {
          let that = this;
          return function () {
              if (that.asyncLoading) {
                  return that.loadImg ? 'background:url(/loading.gif) 0 0 no-repeat' : ''
              }
          }
      },
      aClass() {
      ...
      tree: {
        handler(oldV, newV) {
          if (newV.checked)
            ...
            this.tree.hasChildren=newV.hasChildren;
        },
        deep: true
      },
    }
...
<style>
  @import "../../styles/ztreecss/zTreeStyle/zTreeStyle.css";
</style>
